...
export default function Experiment12({ onBack, onOpenProfile }) {
  ...
  const handleScoreUpdate = (score, total) => {
    ...
    if (user) {
      if (recordQuizScore) {
        recordQuizScore(12, score, totalQ);
      }
      if (recordExpCompleted) {
        recordExpCompleted(12, {
          score,
          total: totalQ,
  ...
  return (
    <div className="flex-1 flex flex-col min-h-screen bg-slate-50">
      <ExperimentNavbar
        title="Exp 12: Query Knowledge Graphs with Pattern-Based Queries"
        tabs={TABS}
        activeTab={activeTab}
        onTabChange={goTo}
        onBack={onBack}
      ...
      <footer className="bg-white border-t border-slate-200 py-4 text-center text-xs text-slate-500 no-print flex flex-col sm:flex-row items-center justify-center gap-2">
        <div>
          <span className="font-semibold text-slate-700">Query Knowledge Graphs with Pattern-Based Queries</span>
          <span className="mx-2">·</span>
          <span className="font-mono">CS-KGIRS-12 · Knowledge Graphs &amp; IR Virtual Lab</span>
        </div>
