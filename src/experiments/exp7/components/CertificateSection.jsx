...
const EXP_META = {
  ...
  subtitle: 'Complementary BM25 Lexical & Dense Vector Ensemble Systems',
  code: 'CS-KGIRS-07',
  version: '1.0'
};

export default function CertificateSection({ quizScore = 10, totalQuestions = 10, studentInfo = {}, onInfoChange, onNext }) {
  ...
  const fields = [
    ...
    { key: 'instructor',  label: 'Faculty Instructor',       icon: Star,       placeholder: 'e.g. Dr. S. Sengupta' },
  ];
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8 space-y-6">
      ...
      <motion.div
        ...
        className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm no-print space-y-4"
      >
          ...
          {fields.map(f => (
            <div key={f.key}>
              ...
              <input
                ...
                className="w-full px-3.5 py-2.5 rounded-xl bg-white border border-slate-200 text-sm text-slate-800 placeholder-slate-300 focus:outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 transition"
              />
            </div>
          ))}
        </div>
        <div className="flex flex-wrap gap-3 pt-1">
          <motion.button
            ...
            className="flex items-center gap-2 px-5 py-2.5 rounded-full bg-gradient-to-r from-indigo-600 to-purple-600 text-white text-sm font-semibold shadow-lg shadow-indigo-200 cursor-pointer"
          >
      ...
      <motion.div
        ...
        className="bg-white rounded-3xl border-2 border-indigo-100 shadow-xl overflow-hidden cert-card"
      >
        {/* Gradient header */}
        <div className="bg-gradient-to-r from-indigo-600 via-purple-600 to-violet-600 px-8 py-8 text-white relative overflow-hidden">
          <div className="absolute -top-10 -right-10 w-40 h-40 rounded-full bg-white/5" />
          <div className="absolute -bottom-6 -left-6 w-28 h-28 rounded-full bg-white/5" />

          <div className="relative flex items-start justify-between gap-4">
            <div className="flex-1">
              <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-indigo-200 mb-2">Certificate of Completion</p>
              <h1 className="text-xl sm:text-2xl font-bold leading-tight mb-1">{EXP_META.title}</h1>
              <p className="text-indigo-200 text-xs leading-relaxed">{EXP_META.subtitle}</p>
            </div>
            {/* Grade badge */}
        ...
        <div className="px-8 py-6 space-y-6">
          ...
          <div className="text-center py-2">
            ...
            <p className="text-2xl font-bold text-slate-900 border-b-2 border-dashed border-indigo-200 pb-2 inline-block min-w-[200px]">
            ...
            <p className="text-sm font-semibold text-indigo-700 mt-0.5">Hybrid BM25 Lexical and Dense Semantic Vector Retrieval</p>
          </div>
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-slate-400 text-center">
              {pct >= 70 ? '✓ Demonstrated proficiency in BM25 parameter tuning, dense embedding similarity, and convex score fusion.' : 'Review theory and simulation to reinforce hybrid retrieval concepts.'}
            </p>
          </div>
          ...
          <div className="flex items-center justify-between text-xs text-slate-400">
            <span className="font-mono bg-slate-50 border border-slate-200 px-2.5 py-1 rounded-lg">
              {EXP_META.code} · v{EXP_META.version}
            </span>
          </div>
          ...
          <div className="flex items-end justify-between pt-2">
            ...
            <div className="text-center">
              <div className="w-14 h-14 mx-auto bg-gradient-to-br from-indigo-600 to-purple-600 rounded-2xl flex items-center justify-center mb-1 shadow-lg shadow-indigo-200">
