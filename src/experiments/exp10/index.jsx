...
export default function Experiment10({ onBack, onOpenProfile }) {
  ...
  const handleScoreUpdate = (score, total) => {
    setQuizScore(score);
    const totalQ = total || QUIZ_QUESTIONS?.length || 12;

    // Persist score & completion status to Cloud Firestore
    if (user) {
      if (recordQuizScore) {
        recordQuizScore(10, score, totalQ);
      }
      if (recordExpCompleted && (score / totalQ) >= 0.7) {
        recordExpCompleted(10, {
          score,
          total: totalQ,
  ...
  return (
    <div className="flex-1 flex flex-col min-h-screen bg-slate-50">
      <ExperimentNavbar
        title="Exp 10: Create and Manage a Graph Database"
        tabs={TABS}
        activeTab={activeTab}
        onTabChange={goTo}
        onBack={onBack}
        quizScore={quizScore}
        totalQuestions={QUIZ_QUESTIONS?.length || 12}
      ...
      <footer className="bg-white border-t border-slate-200 py-4 text-center text-xs text-slate-500 no-print flex flex-col sm:flex-row items-center justify-center gap-2">
        <div>
          <span className="font-semibold text-slate-700">Create and Manage a Graph Database</span>
          <span className="mx-2">·</span>
          <span className="font-mono">CS-KG-10 · Knowledge Graphs Virtual Lab</span>
        </div>
